...
type AdminContentVideoDeleteConfirmationProps = {
  moduleId: string;
...
export function AdminContentVideoDeleteConfirmation({
  moduleId,
  onUpdated,
  videoId,
  videoTitle,
}: AdminContentVideoDeleteConfirmationProps) {
  ...
  async function deleteVideo() {
    ...
    setError("");
    setIsDeleting(true);
    const result = await AdminContentService.deleteVideo(moduleId, videoId);

    if (result.ok) {
      setIsConfirming(false);
      onUpdated(result.module);
    } else {
      setError(result.errors[0]?.message ?? "No fue posible eliminar el video.");
    }
  ...
  return (
    <div className="rounded-xl border border-red-200/40 bg-[var(--color-card-bg)] p-4">
      <p className="text-sm leading-6 text-[var(--color-text-secondary)]">
        ...
        módulo ni el progreso registrado.
      </p>
